...
# engine = get_engine("./checkpoints/model_folded.onnx", engine_model_path)
...
def save_depth():
    ...
    model.load_state_dict(state_dict['model'])  # ['model']
    model.eval()

   # state_dict = torch.load("./checkpoints/model_no_parall.ckpt")
    #state_dict = torch.load("./checkpoints/no_feature_model.ckpt")
    # new_state_dict = OrderedDict()
    # for k, v in state_dict['model'].items():
    #     name = k[7:]  # remove `module.`
    #     new_state_dict[name] = v
    # torch.save(new_state_dict,"./checkpoints/model_no_parall.ckpt")


    #dec feature model -------------------

    # feature_dict = model.feature.state_dict()
    # mod_dic = {k:v for k,v in state_dict.items() if k.split('.')[0] != 'feature'}
    # torch.save(mod_dic,"./checkpoints/no_feature_model.ckpt")
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            #
            # inputs[0].host = np.array(sample_cuda["imgs"]['stage_0'].cpu(),dtype=np.float32)
            # inputs[1].host = np.array(sample_cuda["proj_matrices"]['stage_1'].cpu(),dtype=np.float32)
            # inputs[2].host = np.array(sample_cuda["proj_matrices"]['stage_2'].cpu(),dtype=np.float32)
            # inputs[3].host = np.array(sample_cuda["proj_matrices"]['stage_3'].cpu(),dtype=np.float32)
            # inputs[4].host = np.array(sample_cuda["depth_min"].cpu(),dtype=np.float32)
            # inputs[5].host = np.array(sample_cuda["depth_max"].cpu(),dtype=np.float32)
            # #
            # #
            #
            ...
            # out2 = torch.tensor(trt_outputs[1]).view(1,600,800)
            # out1 = torch.tensor(trt_outputs[0]).view(1,1200, 1600)
            # out2 = torch.tensor(trt_outputs[1]).view(1200, 1600)

            # torch.onnx.export(model, (sample_cuda["imgs"], sample_cuda["proj_matrices"],
            #                 sample_cuda["depth_min"], sample_cuda["depth_max"]), "./checkpoints/model.onnx")
            set_quantizer_by_name(model, ['PatchMatch'], _disabled=True)
            ...
            #                 sample_cuda["depth_min"], sample_cuda["depth_max"])
            if 1:
                # depth = torch.tensor(out2,device="cuda")
                # img = torch.unbind(sample_cuda["imgs"]['stage_0'], 1)[0]
                # depth_min = torch.tensor(sample_cuda["depth_min"],dtype=torch.float32)
                # depth_max = torch.tensor(sample_cuda["depth_max"],dtype=torch.float32)
                # depth = model.upsample_net(img, depth, depth_min, depth_max)
                # score = out1
                # num_depth = model.patchmatch_num_sample[0]
                # score_sum4 = 4 * F.avg_pool3d(F.pad(score.unsqueeze(1), pad=(0, 0, 0, 0, 1, 2)), (4, 1, 1), stride=1,
                #                               padding=0).squeeze(1)
                # # [B, 1, H, W]
                # depth_index = model.depth_regression(score, depth_values=torch.arange(num_depth, device=score.device,
                ...
                #
                # outputs1 = tensor2numpy(depth)
                # outputs2 = tensor2numpy(photometric_confidence)
            ...
            # for filename ,i in zip(filenames,outputs1):
            #     depth_est = outputs1
            #     photometric_confidence =outputs2
                ...
                # photometric_confidence = np.squeeze(photometric_confidence, 0)
...
def set_quantizer_by_name(model, names, **kwargs):
    ...
    for name, mod in model.named_modules():
        if hasattr(mod, 'PatchMatch') or hasattr(mod, 'PatchMatch'):
            for n in names:
                if re.search(n, name):
