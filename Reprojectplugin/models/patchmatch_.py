...
class DepthInitialization(nn.Module):
    def __init__(self, patchmatch_num_sample = 1):
        ...
        self.intial_depth = torch.load("./checkpoints/intial_depth.pt")
...
class Evaluation(nn.Module):
    ...
    def differentiable_warping(self,src_fea, src_proj, ref_proj, depth_samples):
        # src_fea: [B, C, H, W]
        # src_proj: [B, 4, 4]
        # ref_proj: [B, 4, 4]
        ...
        with torch.no_grad():
            if self.stage == 0:
                intr_mat = torch.tensor([[2892.33008,0,823.20398],[0,2883.16992,619.07001],[0,0,1]],device="cuda")
                intr_mat_inv = torch.tensor([[3.4574e-4, 0, -0.28461620],[0, 3.4684e-4, -0.21471854],[0, 0, 1]],
                                            device="cuda")
            elif self.stage == 1:
                intr_mat = torch.tensor([[1446.16504,0,411.60199],[0,1441.58496,309.53500],[0,0,1]],device="cuda")
                intr_mat_inv = torch.tensor([[6.9148e-4, 0, -0.28461620],[0, 6.9368e-4, -0.21471854],[0, 0, 1]],
                                            device="cuda")
            elif self.stage == 2:
                intr_mat = torch.tensor([[723.08252,0,205.80099],[0,720.79248,154.76750],[0,0,1]],device="cuda")
                intr_mat_inv = torch.tensor([[1.38297e-3, 0, -0.28461620],[0, 1.38736e-3, -0.21471854],[0, 0, 1]],
                                            device="cuda")
            elif self.stage == 3:
                intr_mat = torch.tensor([[361.54126,0,102.90050],[0,360.39624,77.38375],[0,0,1]],device="cuda")
                intr_mat_inv = torch.tensor([[2.76594e-3,0,-0.28461620],[0,2.77472e-3,-0.21471854],[0,0,1]],device="cuda")
        ...
        #
        #     rot_ = proj[:, :3, :3]  # [B,3,3]
        #     trans_ = proj[:, :3, 3:4]  # [B,3,1]
        #
        #     y, x = torch.meshgrid([torch.arange(0, height, dtype=torch.float32, device=src_fea.device),
        #                            torch.arange(0, width, dtype=torch.float32, device=src_fea.device)])
        #     y, x = y.contiguous(), x.contiguous()
        #     y, x = y.view(height * width), x.view(height * width)
        #     xyz_ = torch.stack((x, y, torch.ones_like(x)))  # [3, H*W]
        #     xyz_ = torch.unsqueeze(xyz_, 0).repeat(batch, 1, 1)  # [B, 3, H*W]
        #     rot_xyz_ = torch.matmul(rot_, xyz_)  # [B, 3, H*W]
        #
        #     rot_depth_xyz_ = rot_xyz_.unsqueeze(2).repeat(1, 1, num_depth, 1) * depth_samples.view(batch, 1, num_depth,
        #                                                                                          height * width)  # [B, 3, Ndepth, H*W]
        #     proj_xyz_ = rot_depth_xyz_ + trans_.view(batch, 3, 1, 1)  # [B, 3, Ndepth, H*W]
        #     # avoid negative depth
        #     negative_depth_mask = proj_xyz_[:, 2:] <= 1e-3
        #     proj_xyz_[:, 0:1][negative_depth_mask] = width
        #     proj_xyz_[:, 1:2][negative_depth_mask] = height
        #     proj_xyz_[:, 2:3][negative_depth_mask] = 1
        #     proj_xy = proj_xyz_[:, :2, :, :] / proj_xyz_[:, 2:3, :, :]  # [B, 2, Ndepth, H*W]
        ...
        torch.cuda.synchronize(0)
        time2 = time.time()
        print("difftiem:{:.4f}".format((time2-time1)*1000))
    ...
    def forward(self, ref_feature:torch.Tensor, src_features, ref_proj, src_projs, depth_sample, depth_min, depth_max, iter, grid=None, weight=None):
        ...
        if self.stage == 3:
            ...
            for src_feature, src_proj in zip(src_features, src_projs):
                ...
                warped_feature = self.differentiable_warping(src_feature, src_proj, ref_proj, depth_sample)
            ...
            torch.cuda.synchronize(0)
            time3 = time.time()
            #print("1defferential warp for time1:{:.4f} time2:{:.4f} time3:{:.4f} ".format((time2_o - time1_o) * 1000,(time2 - time1) * 1000,(time3 - time2) * 1000))
...
class PatchMatch(nn.Module):
    ...
    #fine tune offset---------jiangpf-----------
    def get_finetune_offset(self,mode:str,offset,device):
        if mode == "propagation":
            if self.propagate_neighbors == 4:
                ...
                original_offset = [[-self.dilation, -self.dilation], [-self.dilation, 0],
    ...
    def get_evaluation_grid(self, batch, height, width, offset, device, img=None):
        ...
        with torch.no_grad():
            y_grid, x_grid = torch.meshgrid([torch.arange(0, height, dtype=torch.float32, device=device),
                                torch.arange(0, width, dtype=torch.float32, device=device)])
            # x_grid, y_grid = cupy.array(cupy.meshgrid(cupy.arange(0, width, dtype=cupy.float32),
            #                                           cupy.arange(0, height, dtype=cupy.float32)))
            # y_grid = from_dlpack(toDlpack(y_grid))
            # x_grid = from_dlpack(toDlpack(x_grid))
            y_grid, x_grid = y_grid.contiguous(), x_grid.contiguous()
            y_grid, x_grid = y_grid.view(height * width), x_grid.view(height * width)
            xy = torch.stack((x_grid, y_grid))  # [2, H*W]
            xy = torch.unsqueeze(xy, 0).repeat(batch, 1, 1)  # [B, 2, H*W]
    ...
    def forward(self, ref_feature, src_features, ref_proj, src_projs, depth_min, depth_max,
        ...
        if self.propagate_neighbors > 0:
            ...
            if not (self.stage == 1 and self.patchmatch_iteration == 1):
                ...
                ###----------------jiangpf_add--------------
                min = propa_offset.min(axis = 1)[0].unsqueeze(1)
                max = propa_offset.max(axis = 1)[0].unsqueeze(1)
                propa_offset = torch.round(((propa_offset - min)*9/(max - min)))
                ###----------------
        ...
        ###----------------jiangpf_add--------------
        min = eval_offset.min(axis=1)[0].unsqueeze(1)
        max = eval_offset.max(axis=1)[0].unsqueeze(1)
        eval_offset = torch.round((eval_offset - min) * 9 / (max - min))
        ###----------------
        #eval_grid = self.get_evaluation_grid(batch,height,width,eval_offset,device,img)

        feature_weight = self.feature_weight_net(ref_feature.detach(),eval_offset) #eval_offset
        ...
        if self.random_initialization:
            ...
            weight = self.depth_weight1(depth_sample.detach(), depth_min, depth_max, eval_offset.detach(), self.patchmatch_interval_scale,
        ...
        #     time3_1 = time.time()
            ...
            weight = self.depth_weight2(depth_sample1.detach(), depth_min, depth_max, eval_offset.detach(), self.patchmatch_interval_scale,
                                    self.evaluate_neighbors)
            weight = weight * feature_weight.unsqueeze(1)
            # weight = weight / torch.sum(weight, dim=2).unsqueeze(2)
            # torch.cuda.synchronize(0)
            ...
            # torch.cuda.synchronize(0)
            # time3_4 = time.time()
            # print("patch time3_1:{:.4f} time3_2:{:.4f} time3_3:{:.4f} all:{:.4f}".format((time3_2 - time3_1) * 1000,
            #                                                                        (time3_3 - time3_2) * 1000,
            #                                                                        (time3_4 - time3_3) * 1000,
            #                                                                        (time3_4 - time3_1) * 1000))
